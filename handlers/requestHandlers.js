...
let usersData = lib.fromJSON(lib.getFileContents(usersRepoPath));
//-----------------------------handlers---------------------------------//
...
const editItem = function(req, res) {
  ...
  let todo = user.fetchTodo(lib.getTodoId(req));
  todo = lib.retriveBehaviour(Todo, todo);
  todo.editItem(lib.getItemId(req), req.body.item);
  user.replaceTodo(todo);
  lib.updateData(usersData, usersRepoPath);
  viewItems(req, res);
};
...
const deleteItems = function(req, res) {
  ...
  let todo = user.fetchTodo(lib.getTodoId(req));
  todo = lib.retriveBehaviour(Todo, todo);
  todo.deleteItem(lib.getItemId(req));
  user.replaceTodo(todo);
  lib.updateData(usersData, usersRepoPath);
  res.sendStatus(201);
  res.end();
...
const markItemDone=function (req,res) {
  ...
  let todo = user.fetchTodo(lib.getTodoId(req));
  todo = lib.retriveBehaviour(Todo, todo);
  todo.markItemAsDone(lib.getItemId(req));
  user.replaceTodo(todo);
  lib.updateData(usersData, usersRepoPath);
  res.sendStatus(201);
  res.end();
